...
2. Clip all shared layers to the county boundary using `sf::st_intersection()`
3. Call `build_current_lyrs()` to produce the four output layers
4. Save each layer as `<layer>_<county>.RData` in `data/02_current_layers/`

Clipping to the county boundary before processing reduces memory and computation time, and ensures that spatial operations in `build_current_lyrs()` do not need to handle the full seven-county extent.
...
- **Xeric soils** (gridcode 100): well-drained upland soils → candidate upland habitats
- **Non-xeric soils** (gridcode 200 or 300): seasonally or permanently saturated soils → candidate wetland habitats

### Stage 2 — Tidal position

Each soil group is then split by intersection with the coastal stratum:

- Xeric soils **inside** the coastal stratum → `Coastal Uplands`
- Xeric soils **outside** the coastal stratum → `Native Uplands`
- Non-xeric soils **inside** the coastal stratum → tidal wetlands (further classified below)
- Non-xeric soils **outside** the coastal stratum → `Freshwater Wetlands`
...
- Tidal wetlands in the **0.5–18 psu zone** → `Salt Marshes` (dominated by *Juncus* and *Spartina*; the 18 psu boundary follows @eleuterius1979)
- Tidal wetlands in **all other salinity zones** (Fresh < 0.5 psu and Euhaline > 18 psu) → `Mangrove Forests/Salt Barrens`
