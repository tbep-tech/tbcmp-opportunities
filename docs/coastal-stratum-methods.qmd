...
The coastal stratum defines the zone of interest for reservation space in the Tampa Bay Habitat Master Plan developed for the Tampa Bay Coastal Master Plan (TBCMP). It represents the land area between Mean Lower Low Water (MLLW) and 5 feet (1.524 m) NAVD88 most likely to be impacted by sea level rise.
...
All functions are executed in `R/01_inputs.R` to produce the final `coastal_stratum` object.