...
The reservation categories cover the coastal stratum opportunity space; the proposed conservation categories cover non-coastal proposed lands. Proposed conservation features within the coastal stratum are excluded to avoid double-counting with the reservation layers.
...
The six opportunity categories use fixed colours consistent with the hmpu-workflow ggplot2 static maps:
...
| `oppmap_leaflet()` | Format `oppdat_fun()` output into an interactive `leaflet` map for reporting |
