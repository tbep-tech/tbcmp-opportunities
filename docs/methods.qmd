...
MacDill Air Force Base is excluded from this layer as it is not a conservation land use. The filtered layer is saved to `data/flma.RData`.
...
The layer is saved to `data/ffbot.RData`.
...
The layer is saved to `data/ffa.RData`.
...
The GeoJSON is routed through GDAL `vectortranslate` to linearize any curve geometries before reading. The layer is saved to `data/aqprs.RData`.
...
The zip archive is deleted after processing. The final layer is saved to `data/clip.RData`.
...
Both layers are cast to `POLYGON` geometry and saved to `data/exst.RData` and `data/prop.RData` respectively.
...
| File | Object |
|---|---|
| `data/lulc_citrus.RData` | `lulc_citrus` |
| `data/lulc_hernando.RData` | `lulc_hernando` |
| `data/lulc_hillsborough.RData` | `lulc_hillsborough` |
| `data/lulc_manatee.RData` | `lulc_manatee` |
| `data/lulc_pasco.RData` | `lulc_pasco` |
| `data/lulc_pinellas.RData` | `lulc_pinellas` |
| `data/lulc_sarasota.RData` | `lulc_sarasota` |
