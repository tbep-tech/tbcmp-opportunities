...
The combined layer is then clipped to each of the seven county boundaries (supplied via the `tbcmp_cnt` argument) using `sf::st_intersection()` and saved as a separate RData file:
...
| Function | Purpose |
|---|---|
| `fetch_seagrass(tbcmp_cnt)` | Download, combine, clip, and save all seven county seagrass layers |
